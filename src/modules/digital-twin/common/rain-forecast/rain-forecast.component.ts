...
export class RainForecastComponent implements AfterViewInit {
  ...
  protected rainForecastModalOpen: signals.ToggleableSignal =
    signals.toggleable(false);
  ...
  updateLengthForecastModal() {
    ...
    if (length < duration) {
      ...
      const newArray = Array.from({length: duration - length}, (_, i) => ({
        time: ((i + 1 + length) * factor).toString(),
        rainAmount: 0,
        waterLevel: 0,
      }));
  ...
  setForecastModalFromTime(time: string) {
    console.log("Setting forecast modal from time:", time);
